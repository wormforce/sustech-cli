...
export const CONSEQUENCES: readonly Consequence[] = [
  ...
  consequence("profile.export", "medium", true, "Writes a whitelisted student profile report to an explicit local JSON path.", "The report contains personal academic data, and --overwrite can replace an existing local file.", "Verify the returned absolute path, then inspect the saved file's private mode, schema version, masked SID, and per-source statuses.", "implemented"),
  consequence("academic.snapshot.save", "medium", true, "Writes a versioned academic-state snapshot to an explicit local JSON path.", "The snapshot contains personal academic data, and --overwrite can replace an existing local file.", "Verify the returned absolute path, private file mode, schema version, digest, and per-source statuses.", "implemented"),
