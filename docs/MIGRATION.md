...
| TIS planning | persistent week-aware timetable plans, scored solver, read-only `tis plan explain/recommend`, blocked periods, conservative local degree audit | guarded local plan files | Implemented with fixtures; `explain/recommend` stay advisory and never select courses automatically | P1 |
...
| Context and profile | truthful calendar/live-class/exam/deadline context; Context v2 `--level`/`--live`; whitelisted student summary with independent source status | guarded versioned profile export | Implemented with conservative omissions and masked identity fields | P1 |
...
3. Exercise both direct and `--browser` Primo transports on more hosts and keep the browser path as the manual-auth fallback.
